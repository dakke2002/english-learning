...
// Sustainable IELTS path. These lessons intentionally use different prompts,
// language targets and model answers instead of cloning the mock lesson.
const makeIeltsLesson = (config: {
  id: string; title: string; titleEn: string; description: string; icon: string;
  vocabulary: Vocabulary[]; patterns: SentencePattern[]; english: string; chinese: string;
  exercises: DictationExercise[];
}): SubCourse => ({
  id: config.id, title: config.title, titleEn: config.titleEn, titleChinese: config.title,
  description: config.description, icon: config.icon, videoFile: '', vocabulary: config.vocabulary,
  sentencePatterns: config.patterns, bilingualText: { english: config.english, chinese: config.chinese },
  englishText: config.english, dictationExercises: config.exercises,
});

const shortVocab = (items: Array<[string, string, string]>): Vocabulary[] => items.map(([word, meaning, example]) => ({ word, phonetic: '', meaning, example }));
const pattern = (english: string, chinese: string, usage: string): SentencePattern => ({ english, chinese, usage });
const gap = (sentence: string, answer: string, hint: string): DictationExercise => ({ sentence, answer, hint });

const additionalIeltsLessons: SubCourse[] = [
  makeIeltsLesson({ id: 'ielts-speaking-06', title: '基础：把短答案说完整', titleEn: 'Foundation: Extend a Short Answer', description: '用 TS + Reason + Example，把一句话扩展成自然的 20 秒回答。', icon: '01', vocabulary: shortVocab([['routine','日常习惯','My morning routine keeps me focused.'],['occasionally','偶尔','I occasionally study in a cafe.'],['practical','实用的','It is a practical way to practise speaking.'],['commute','通勤','I listen to English on my commute.'],['reliable','可靠的','A reliable routine is easier to maintain.']]), patterns: [pattern('The main reason is that...', '主要原因是……','解释原因'), pattern('For example, ...', '例如……','补充具体例子'), pattern('That is why...', '这就是为什么……','收束结果'), pattern('To be honest, ...', '说实话……','自然表达态度')], english: 'I usually practise speaking in the evening because my mind is clearer after work. For example, I record a two-minute answer about my day and listen for repeated words. It is a practical routine, and I can maintain it even when I am busy. That is why short, regular practice works better for me than studying once a week.', chinese: '我通常在晚上练习口语，因为工作后思路更清晰。例如，我会录下两分钟的日常回答，再检查重复用词。这是一个实用的习惯，即使忙的时候也能坚持。因此，对我来说，短时间的规律练习比一周学一次更有效。', exercises: [gap('I usually practise speaking in the ___.','evening','晚上'), gap('My mind is clearer after ___.','work','工作后'), gap('I record a ___ answer.','two-minute','两分钟的'), gap('It is a practical ___.','routine','习惯')]}),
  makeIeltsLesson({ id: 'ielts-speaking-07', title: 'Part 1：过去、现在与未来', titleEn: 'Part 1: Past, Present and Future', description: '练习 Part 1 常见时态切换，避免只会使用现在时。', icon: '02', vocabulary: shortVocab([['recently','最近','I recently changed my study plan.'],['used to','过去常常','I used to avoid speaking in public.'],['currently','目前','I am currently working on pronunciation.'],['eventually','最终','I will eventually feel more confident.'],['intention','打算','My intention is to practise daily.']]), patterns: [pattern('I used to..., but now...', '我过去……但现在……','过去与现在对比'), pattern('At the moment, ...', '目前……','当前状态'), pattern('In the future, I hope to...', '未来我希望……','未来计划'), pattern('Once I have..., I will...', '一旦我……我就会……','条件与计划')], english: 'I used to avoid speaking English in front of other people, but now I practise with a partner twice a week. At the moment, I am working on pronunciation and longer answers. In the future, I hope to join an English-speaking club. Once I have built more confidence, I will use English at work as well.', chinese: '我过去避免在别人面前说英语，但现在每周和同伴练习两次。目前我正在练发音和更长的回答。未来我希望加入英语交流社团。等我建立更多信心后，也会在工作中使用英语。', exercises: [gap('I ___ to avoid speaking English.','used','过去常常'), gap('I practise with a ___ twice a week.','partner','同伴'), gap('At the moment, I am working on ___.','pronunciation','发音'), gap('I hope to join an English-speaking ___.','club','社团')]}),
  makeIeltsLesson({ id: 'ielts-speaking-08', title: 'Part 2：描述一件重要物品', titleEn: 'Part 2: Describe an Important Object', description: '用来源、外观、使用场景和情感价值完成物品类 Cue Card。', icon: '03', vocabulary: shortVocab([['well-worn','磨旧的','It is a well-worn notebook.'],['handwritten','手写的','The pages are full of handwritten notes.'],['reminder','提醒物','It is a reminder of my progress.'],['portable','便携的','The notebook is small and portable.'],['sentimental','有情感价值的','It has sentimental value for me.']]), patterns: [pattern('I would like to describe...', '我想描述……','开场定位'), pattern('What makes it special is...', '它特别之处在于……','突出特点'), pattern('Whenever I..., I...', '每当我……我就……','使用场景'), pattern('Looking back, ...', '回头看……','总结意义')], english: 'I would like to describe a small notebook that my first English teacher gave me. It is plain and well-worn, but the pages are full of handwritten phrases and corrections. Whenever I feel stuck, I open it and choose one sentence to practise aloud. What makes it special is that it shows my progress over several years. Looking back, it has become a portable reminder that improvement comes from small steps.', chinese: '我想描述一本第一位英语老师送给我的小笔记本。它很普通，也已经磨旧了，但里面写满了短语和修改记录。每当我卡住时，我就打开它，挑一句大声练习。它特别之处在于记录了我几年来的进步。现在回头看，它提醒我进步来自每天的小步坚持。', exercises: [gap('It is a small ___.','notebook','笔记本'), gap('The pages are full of handwritten ___.','phrases','短语'), gap('I practise one sentence ___.','aloud','大声地'), gap('It is a reminder of my ___.','progress','进步')]}),
  makeIeltsLesson({ id: 'ielts-speaking-09', title: 'Part 2：讲清一段难忘经历', titleEn: 'Part 2: Tell a Memorable Experience', description: '用时间线、转折和结果，让故事听起来连贯而不是罗列细节。', icon: '04', vocabulary: shortVocab([['unexpected','意外的','The result was completely unexpected.'],['overwhelmed','不知所措的','I felt overwhelmed at first.'],['determined','坚定的','I became determined to finish it.'],['turning point','转折点','It was a turning point for me.'],['grateful','感激的','I am still grateful for the support.']]), patterns: [pattern('At first, ...', '起初……','故事开头'), pattern('The turning point came when...', '转折发生在……','转折'), pattern('As a result, ...', '结果……','结果'), pattern('It taught me that...', '这让我明白……','反思')], english: 'I would like to talk about the first time I gave a presentation in English. At first, I felt overwhelmed and spoke too quickly, so I lost my place halfway through. The turning point came when a classmate smiled and encouraged me to pause. I took a breath, used my notes, and finished the presentation. As a result, I became determined to practise public speaking. It taught me that recovering calmly is more useful than trying to be perfect.', chinese: '我想说说第一次用英语做演讲的经历。起初我不知所措，说得太快，中途忘了内容。转折发生在同学微笑着鼓励我停顿之后。我深吸一口气，看着笔记完成了演讲。结果我决心练习公开表达。这让我明白，冷静地修复比追求完美更有用。', exercises: [gap('I gave a presentation in ___.','English','英语'), gap('I felt ___ at first.','overwhelmed','不知所措的'), gap('A classmate encouraged me to ___.','pause','停顿'), gap('I became determined to practise public ___.','speaking','表达')]}),
  makeIeltsLesson({ id: 'ielts-speaking-10', title: 'Part 2：描述生活中的变化', titleEn: 'Part 2: Describe a Change', description: '用过去与现在对比，说明变化的原因、影响和个人态度。', icon: '05', vocabulary: shortVocab([['noticeable','明显的','The change is noticeable.'],['gradually','逐渐地','My confidence gradually improved.'],['adapt','适应','People need time to adapt.'],['convenient','方便的','Online lessons are more convenient.'],['drawback','缺点','The main drawback is less face-to-face contact.']]), patterns: [pattern('Compared with the past, ...', '与过去相比……','对比'), pattern('This change happened because...', '这一变化发生是因为……','原因'), pattern('The biggest impact has been...', '最大的影响是……','影响'), pattern('Although..., I still...', '虽然……我仍然……','平衡观点')], english: 'I would like to describe how I changed the way I study English. Compared with the past, I now use short online lessons and record myself every day. This change happened because my schedule became less predictable. The biggest impact has been greater flexibility, although the main drawback is less face-to-face contact. Overall, I have adapted well because the routine is convenient and the progress is easy to track.', chinese: '我想描述自己学习英语方式的变化。与过去相比，我现在使用短时在线课程，并每天录音。这一变化是因为我的时间安排变得不稳定。最大的影响是更加灵活，缺点是面对面交流少了。总体来说我适应得不错，因为这种习惯方便，而且进步容易追踪。', exercises: [gap('I use short ___ lessons.','online','在线的'), gap('My schedule became less ___.','predictable','可预测的'), gap('The biggest impact is greater ___.','flexibility','灵活性'), gap('I have ___ well.','adapted','适应')]}),
  makeIeltsLesson({ id: 'ielts-speaking-11', title: 'Part 3：比较与评价', titleEn: 'Part 3: Compare and Evaluate', description: '从个人偏好上升到群体差异，完成两组 TS + SD 分析。', icon: '06', vocabulary: shortVocab([['accessible','容易获得的','Digital resources are widely accessible.'],['interactive','互动的','Classroom learning is more interactive.'],['efficient','高效的','Online practice can be efficient.'],['relevant','相关的','Teachers can give relevant feedback.'],['trade-off','权衡','There is a trade-off between cost and support.']]), patterns: [pattern('The main difference is that...', '主要区别是……','比较'), pattern('This tends to be more...', '这往往更……','评价'), pattern('The trade-off is...', '需要权衡的是……','限制'), pattern('From a broader perspective, ...', '从更广的角度看……','上升到社会')], english: 'The main difference between online and classroom speaking practice is the type of support learners receive. Online practice is more accessible and efficient because students can repeat a task whenever they want. Classroom learning, by contrast, is more interactive, so teachers can notice misunderstandings immediately. From a broader perspective, the best choice depends on the learner’s self-discipline. There is a trade-off between flexibility and direct feedback, rather than one method being perfect for everyone.', chinese: '在线和课堂口语练习的主要区别在于学习者得到的支持不同。在线练习更容易获得，也更高效，因为学生可以随时重复任务。相比之下，课堂学习互动性更强，老师能马上发现误解。从更广的角度看，选择取决于学习者的自律性。灵活性和直接反馈之间存在权衡，并不存在适合所有人的完美方法。', exercises: [gap('Online practice is more ___ and efficient.','accessible','容易获得的'), gap('Classroom learning is more ___.','interactive','互动的'), gap('The best choice depends on self-___.','discipline','自律'), gap('There is a trade-off between flexibility and direct ___.','feedback','反馈')]}),
  makeIeltsLesson({ id: 'ielts-speaking-12', title: 'Part 3：原因与影响', titleEn: 'Part 3: Causes and Consequences', description: '练习 because → result → example 的原因影响链。', icon: '07', vocabulary: shortVocab([['urbanisation','城市化','Urbanisation changes daily travel.'],['pressure','压力','Commuters face considerable pressure.'],['congestion','拥堵','Traffic congestion wastes time.'],['productivity','生产力','Flexible work can improve productivity.'],['ultimately','最终','It ultimately affects wellbeing.']]), patterns: [pattern('One major reason is...', '一个主要原因是……','原因'), pattern('This means that...', '这意味着……','解释结果'), pattern('For instance, ...', '例如……','举例'), pattern('Ultimately, ...', '最终……','总结影响')], english: 'One major reason people prefer flexible work is that commuting has become expensive and stressful. This means that employees may spend hours in congestion before they even begin working. For instance, a parent who works from home can use that time to prepare a child’s breakfast. Flexible work is not suitable for every job, but it can reduce daily pressure and improve productivity. Ultimately, its value depends on how responsibly it is organised.', chinese: '人们更喜欢灵活工作的一个主要原因是通勤变得昂贵而且有压力。这意味着员工可能还没开始工作就已经堵在路上几个小时。例如，在家工作的家长可以用这段时间给孩子准备早餐。灵活工作并不适合所有职业，但能减少日常压力并提高效率。最终，它的价值取决于组织方式是否负责。', exercises: [gap('Commuting has become expensive and ___.','stressful','有压力的'), gap('Employees may spend hours in ___.','congestion','拥堵中'), gap('Flexible work can reduce daily ___.','pressure','压力'), gap('It can improve ___.','productivity','生产力')]}),
  makeIeltsLesson({ id: 'ielts-speaking-13', title: 'Part 3：问题与解决方案', titleEn: 'Part 3: Problems and Solutions', description: '针对现实问题提出可执行、有边界的方案，避免空泛口号。', icon: '08', vocabulary: shortVocab([['barrier','障碍','Cost is a barrier for many learners.'],['initiative','倡议','Schools can launch a local initiative.'],['affordable','负担得起的','Libraries can provide affordable access.'],['measure','措施','This measure would be easy to test.'],['long-term','长期的','Long-term support matters more than publicity.']]), patterns: [pattern('A practical solution would be...', '一个实际的解决方案是……','提出方案'), pattern('This could be done by...', '可以通过……实现','说明执行'), pattern('However, it would only work if...', '但只有在……时才有效','限制条件'), pattern('In the long term, ...', '从长期看……','长期结果')], english: 'A practical solution to unequal access to speaking practice would be free conversation clubs in public libraries. This could be done by training volunteers and providing a simple weekly topic. However, it would only work if libraries had a small budget for supervision and materials. Schools could also encourage students to attend by recognising regular participation. In the long term, such initiatives would reduce the cost barrier and give learners a supportive place to speak.', chinese: '解决口语练习机会不平等的一个实际方案，是在公共图书馆开设免费交流俱乐部。可以培训志愿者，每周提供一个简单话题。但只有图书馆有少量经费用于管理和材料时，方案才会有效。学校也可以认可持续参加的学生。从长期看，这类倡议能降低费用障碍，为学习者提供支持性的表达场所。', exercises: [gap('A practical solution is free conversation ___.','clubs','俱乐部'), gap('Volunteers need simple weekly ___.','topics','话题'), gap('The plan needs a small ___.','budget','预算'), gap('It would reduce the cost ___.','barrier','障碍')]}),
  makeIeltsLesson({ id: 'ielts-speaking-14', title: '模考二：节奏与修复', titleEn: 'Mock 2: Timing and Recovery', description: '连续完成 Part 1、Part 2、Part 3，训练计时、停顿和自我修正。', icon: '09', vocabulary: shortVocab([['hesitate','犹豫','Try not to hesitate for too long.'],['rephrase','改述','You can rephrase the question.'],['coherent','连贯的','Keep the answer coherent.'],['pace','语速','A steady pace is easier to understand.'],['recover','恢复','Pause briefly and recover.']]), patterns: [pattern('Let me put it another way...', '换一种说法……','改述'), pattern('What I mean is...', '我的意思是……','澄清'), pattern('The key point is...', '关键点是……','拉回主线'), pattern('To sum up, ...', '总结来说……','结束回答')], english: 'During a mock test, I try to keep a steady pace rather than speaking as quickly as possible. If I hesitate, I pause briefly and use a natural phrase such as “Let me put it another way.” When I lose a detail, I return to the key point and continue instead of apologising repeatedly. This helps me recover without breaking coherence. To sum up, timing matters, but clear organisation matters more than rushing.', chinese: '在模考中，我会保持稳定语速，而不是尽可能快地说。如果犹豫，我会短暂停顿，用“换一种说法”这样的自然表达。当我忘记细节时，我会回到关键点继续，而不是反复道歉。这样能在不破坏连贯性的情况下恢复。总之，时间很重要，但清晰组织比赶速度更重要。', exercises: [gap('Keep a steady ___.','pace','语速'), gap('Pause briefly and use a natural ___.','phrase','表达'), gap('Return to the key ___.','point','关键点'), gap('Clear organisation matters more than ___.','rushing','赶速度')]}),
  makeIeltsLesson({ id: 'ielts-speaking-15', title: 'Band 7 复盘：建立个人错误库', titleEn: 'Band 7 Review: Build an Error Bank', description: '用录音复盘四项评分标准，制定下一轮可执行的训练目标。', icon: '10', vocabulary: shortVocab([['pattern','模式','Look for a repeated error pattern.'],['accuracy','准确性','Accuracy improves through correction.'],['intelligible','清晰易懂的','Your pronunciation should be intelligible.'],['specific','具体的','Set one specific weekly target.'],['monitor','监测','Monitor your progress every Friday.']]), patterns: [pattern('One pattern I noticed is...', '我注意到的一个模式是……','指出问题'), pattern('The evidence is...', '证据是……','引用录音'), pattern('My next target is...', '我的下一个目标是……','设定目标'), pattern('I will measure it by...', '我会通过……来衡量','制定指标')], english: 'After reviewing my recordings, one pattern I noticed is that I give clear ideas but often use the same linking words. The evidence is that “because” appears several times in one answer. My next target is to use two different reason linkers in every practice response. I will measure it by highlighting them in my transcript each Friday. This is more useful than trying to improve everything at once because the target is specific and easy to monitor.', chinese: '复盘录音后，我注意到一个模式：观点很清楚，但经常重复使用相同的连接词。证据是同一个回答中多次出现 because。我的下一个目标是在每次练习中使用两种不同的原因连接词。我会每周五在文字稿中标记它们来衡量。这样比一次改进所有问题更有效，因为目标具体且容易监测。', exercises: [gap('I often use the same linking ___.','words','连接词'), gap('The evidence is in my ___.','transcript','文字稿'), gap('My next ___ is specific.','target','目标'), gap('I will ___ my progress weekly.','monitor','监测')]}),
];

ieltsEnglishCourse.subCourses.push(...additionalIeltsLessons);
const completeIeltsOrder = ['ielts-speaking-06', 'ielts-speaking-07', 'ielts-speaking-02', 'ielts-speaking-01', 'ielts-speaking-03', 'ielts-speaking-08', 'ielts-speaking-09', 'ielts-speaking-10', 'ielts-speaking-04', 'ielts-speaking-11', 'ielts-speaking-12', 'ielts-speaking-13', 'ielts-speaking-05', 'ielts-speaking-14', 'ielts-speaking-15'];
ieltsEnglishCourse.subCourses.sort((a, b) => completeIeltsOrder.indexOf(a.id) - completeIeltsOrder.indexOf(b.id));
